...
def ingest_metrics(request_body: str) -> Dict[str, Any]:
    ...
    # Parse request
    body = json.loads(request_body)  # BUG: invalid JSON will crash (500)
    ...
    try:
        resp = inference.fingerprint(project_id, metrics, timeout_ms=cfg["inference_timeout_ms"])
    ...
    try:
        db.save_fingerprint(request_id, project_id, fingerprint_id, created_at=received_at)
    except Exception:
